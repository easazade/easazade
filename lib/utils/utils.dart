...
void logAsFile({required String content, required String fileName}) {
...
String? readLogFile({required String fileName}) {
  final logFile = File(createLogFilePath(fileName));
  if (logFile.existsSync()) {
...
class RepoInfo {
  final String name;
  final String owner;
  final RepoOwnerType ownerType;

  RepoInfo({required this.name, required this.owner, required this.ownerType});
}

enum RepoOwnerType { user, organization }
